...
from src.ground import fetch_ids
from src.recommend import recommend_query


def test_real_query_returns_grounded_cited_recs():
    res = recommend_query(q="upscale italian near river north", lat=41.892, lng=-87.634)
...
def test_impossible_query_fails_honestly():
    ...
    res = recommend_query(q="sushi right here", lat=41.900, lng=-87.600)
    assert res["recommendations"] == []
    assert res["grounded"] is True  # honest empty, not a guess


def test_closed_list_never_invents_ids():
    """Even if ranking is weird, place_ids must stay inside grounding."""
    res = recommend_query(q="italian near river north", lat=41.892, lng=-87.634)
    grounded = set(fetch_ids(41.892, -87.634, "italian"))
    for rec in res["recommendations"]:
        assert rec["place_id"] in grounded
